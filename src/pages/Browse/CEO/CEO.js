...
const CEO = () => {
  const [getCEOBooks, setCEOBooks] = useState([]);

  useEffect(() => {
    GetCEOBooksHandler();
  }, []);

  const GetCEOBooksHandler = async () => {
    let result = await axios.get("http://103.171.181.46:2081/api/books");
    console.log(result.data.data);
    setCEOBooks(result.data.data);
  };
  return (
    <>
      <BrowseNavbar />
      <div className="browse-card">
        <div className="container">
          <div className="row">
            {getCEOBooks.slice(0,3).map((topWerewolfBooks) => (
      ...
      <div className="browse-book-content">
        <div className="container">
          {getCEOBooks.map((wereWolfBooks, index) => (
            <div className="row">
