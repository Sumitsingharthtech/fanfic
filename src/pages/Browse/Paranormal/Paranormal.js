...
const Paranormal = () => {
  const [getParanormalBooks, setParanormalBooks] = useState([]);

  useEffect(() => {
    GetParanormalBooksHandler();
  }, []);

  const GetParanormalBooksHandler = async () => {
    let result = await axios.get("http://103.171.181.46:2081/api/books");
    console.log(result.data.data);
    setParanormalBooks(result.data.data);
  };
    return (
        <>
        <BrowseNavbar />
      <div className="browse-card">
        <div className="container">
          <div className="row">
            {getParanormalBooks.slice(0,3).map((topWerewolfBooks) => (<div className="col-sm-12 col-md-6 col-lg-4">
      ...
      <div className="browse-book-content">
        <div className="container">
          {getParanormalBooks.map((wereWolfBooks, index) => (
            <div className="row">
